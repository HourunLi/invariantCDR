...
#         x_k_abs = x_k.norm(dim=-1)
#         # part 1
#         k_sim = torch.einsum('bid,bjd->bij', x_q, x_k) / (1e-8 + torch.einsum('bi,bj->bij', x_q_abs, x_k_abs))
...
class invariantCDR(nn.Module):
    ...
    def inter_cl(self, x_q, x_k, center_v):
        B, K, d = x_q.size()
        ...
        sim_matrix = torch.einsum('kid,kjd->kij', x_q, x_k) / (1e-8 + torch.einsum('ki,kj->kij', x_q_abs, x_k_abs))
        # sim_matrix = sim_matrix.permute(1, 0, 2).contiguous()
        sim_matrix = F.softmax(sim_matrix / self.inter_tau, dim=-1) # equation 8 in paper
        score = sim_matrix[:, range(B), range(B)]
        p_y_xk = score.view(K, B, 1)
        # (B, K, 1)
        p_y_xk = p_y_xk.permute(1, 0, 2)
        
        # (B, K, 1)
        q_k = p_k_x * p_y_xk
        q_k_xy = q_k/torch.sum(q_k, dim=1, keepdim=True) # equation 9
        elbo = q_k_xy * (torch.log(q_k) - torch.log(q_k_xy))
        loss = -elbo.mean()
        # loss = -nll_loss.mean()
        return loss
    ...
    def intra_cl(self, x_q, x_k):
        ...
        x_q_2_abs = x_q_abs.permute(1, 0).contiguous()
        x_k_2_abs = x_k_abs.permute(1, 0).contiguous()
        sim_matrix = torch.einsum('kid,kjd->kij', x_q_2, x_k_2) / (1e-8 + torch.einsum('ki,kj->kij', x_q_2_abs, x_k_2_abs))
        sim_matrix = F.softmax(sim_matrix / self.intra_tau, dim = -1)
        # sim_matrix = sim_matrix.sum(dim = 0)
        sim_matrix = sim_matrix[:, range(B), range(B)]
        score_2 = sim_matrix.view(B, K)
        loss_2 = -torch.log(score_2).mean()
        # print(f"intra loss: loss_1: {loss_1}, loss_2: {loss_2}")
